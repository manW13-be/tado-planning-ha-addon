...
#
# Keeps the test image intact. Run docker_test_deploy.sh to rebuild.
#
# Usage:
...
if docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q "^${TEST_CONTAINER}$"; then
    log "Stopping and removing container '$TEST_CONTAINER'..."
    docker rm -f "$TEST_CONTAINER" >/dev/null
    log "Done. Image 'addon_test_tado_planning' is preserved."
    log "Rebuild: ./scripts/docker_test_deploy.sh"
else
